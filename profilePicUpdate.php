...
if ($request_method == 'POST') {
    ...
    if (isset($message)) {
        ...
        $query1 = "SELECT profilePic FROM users WHERE id = :member_id";
        $statement1 = $db->prepare($query1);
        $statement1->bindValue(":member_id", $member_id);
        $statement1->execute();
        $result_array1 = $statement1->fetch();
        $statement1->closeCursor();
        $profile_pic_old = $result_array1['profilePic'];
        ...
        if (move_uploaded_file($_FILES["picture"]["tmp_name"], $pic_name)) {
            ...
            $query2 = "UPDATE users SET profilePic = :profile_pic WHERE id = :member_id";
